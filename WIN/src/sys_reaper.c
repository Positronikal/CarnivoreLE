...
/* Capture hardware inventory using msinfo32 */
int
sys_reaper ()
{
  int done;
  ...
  while (!done)
    {
      printf ("Capture hardware inventory? /msinfo32.exe/ [y/n]\n");
      ch = getchar ();
...
sys_r ()
{
  char allsys[PATH_MAX];

  snprintf (allsys, sizeof (allsys), "C:\\Windows\\System32\\msinfo32.exe \
            /report %s\\systeminfo.txt", outputdir);
  system (allsys);
